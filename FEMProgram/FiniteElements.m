...
function [JResults] = FiniteElements(IOFolder, InputFile, OutputFile, dynamicAnalysis)

     tic;
     Results.Metadata.Suceded = 0;
     Results.Metadata.AsError = 0;
     Results.Metadata.ElapsedTime = 0;
     
     Structure=loadjson(fullfile(IOFolder, InputFile)); %%'result.json'
    ...
    for i = 1:length(MpA)
        ...
        Results.Structure.(nodeNumber).Displacements = U(MpA(i):(MpA(i)+1));

        if df(i) == 3
            %fprintf('\t theta_%d = %f', i, U(MpA(i)+2));
            Results.Structure.(nodeNumber).Displacements = U(MpA(i):(MpA(i)+2));
        end
    end
    ...
    for i = 1:max(max(elements))
        ...
        if fixedMovements0(i,1) ~= 0
            %fprintf('\n Node %d: ', i);
            %fprintf('\t xx_%d = %f\n', i, R(MpA(i)));
            Results.Structure.(nodeNumber).Reactions = R(MpA(i));
        end

        if fixedMovements0(i,2) ~= 0
            %fprintf(' Node %d: ', i);
            %fprintf('\t yy_%d = %f\n', i, R(MpA(i)+1));
            Results.Structure.(nodeNumber).Reactions = R(MpA(i):(MpA(i)+1));
        end

        if fixedMovements0(i,3) ~= 0
            %fprintf(' Node %d: ', i);
            %fprintf('\t Mz_%d = %f\n', i, R(MpA(i)+2));
            Results.Structure.(nodeNumber).Reactions = R(MpA(i):(MpA(i)+2));
        end
    end
        ...
        for ii = 1:length(elementType);
            ...
            switch elementType(ii)
                ...
                case 'l'
                    ...
                    Results.Structure.(Element).AxialForce = axialForce;
                    Results.Structure.(Element).AxialStress = axialStress;
                ...
                case 'b'
                    ...
                    Results.Structure.(Element).AxialForce = axialForce;
                    Results.Structure.(Element).shearForce1 = shearForce1;
                    Results.Structure.(Element).shearForce2 = shearForce2;
                    Results.Structure.(Element).moment1 = moment1;
                    Results.Structure.(Element).moment2 = moment2;
            end
        end %% for
    end %% iff
    ...
    Results.Metadata.Suceded = 1;
    Results.Metadata.ElapsedTime = toc;
    JResults = savejson('',Results,fullfile(IOFolder, OutputFile));
